        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var user = await _applicationDbContext.Users.Include(u => u.Roles).ThenInclude(ur => ur.Role)
                        .FirstOrDefaultAsync(u => u.Email == Input.Email);

                    if (user != null)
                    {
                        var jwt = _jwtService.GetJwt(user.Id, user.Roles.FirstOrDefault()?.Role.Name,
                            user.CompanyId.ToString(), user.FirstName, user.LastName, user.Email);
